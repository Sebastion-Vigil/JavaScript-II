...
// businesses to make a small donation on their behalf for 
...
// bibs. Combine both the first and last names and populate a new array called `fullNames`. 
...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because 
// the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. 
// This array will contain just strings.
...
// filtered version of the runners array, containing only those runners with large sized 
// shirts so they can choose a different size. This will be an array of objects.
...
let total = 0;
runners.reduce((current, runner) => {
  total += runner.donation;
})
console.log("Ticket price total: ", total);
...
// Problem 1
// return list of runners with donations above $100
const aboveAHundred = [];
runners.forEach((runner) => {
  if (parseInt(runner.donation) >= 100) {
    aboveAHundred.push(runner.first_name)
  }
})
console.log(aboveAHundred);
// Problem 2
